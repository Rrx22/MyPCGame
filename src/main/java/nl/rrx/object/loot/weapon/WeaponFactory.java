...
public enum WeaponFactory {
    ...
    public Weapon create() {
            if (this.title.contains("Axe")) {
            return new Axe(image, minAttack, maxAttack, title, description);
        }
        if (this.title.contains("Sword")) {
            return new Sword(image, minAttack, maxAttack, title, description);
        }
        throw new RuntimeException("Unsupported title: " + title);
    }
}
